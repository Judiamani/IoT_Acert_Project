...
class Decompressor:
    ...
    def build_ipv6_pseudo_header(self):
        ...
        print("entete ipv6 decompressé",phdr)
        return phdr
    ...
    def decompress(self, schc, rule, direction):
        ...
        for r in rule["Compression"]:
            ...
            if r[T_DI] in [T_DIR_BI, direction]:
                ...
                dprint("<<< ic", full_field)
                self.parsed_packet[(r[T_FID], r[T_FP])] = full_field
                pprint.pprint (self.parsed_packet)
                print("nombre de header du packet reconstitué",len(self.parsed_packet)+1)
        b=self.parsed_packet
        
        print("type du packet de fin",type(b))
        print("taille de fin",b.__len__)
